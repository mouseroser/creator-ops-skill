...
**main 的角色**：编排中心 + 监控，不直接执行发布脚本。wemedia subagent 获取内容后直接调用 media-tools 完成发布。

**发布主体**：wemedia subagent 直接调用 `python3 scripts/publish_pipeline.py`（已验证可独立发布）。
...
| 配图 | `~/.openclaw/workspace/agents/wemedia/drafts/generated/{A\|B\|C}/{标识}_sq.jpg` | main / nanobanana | main / media-tools |
| 发布命令 | `cd ~/.openclaw/skills/media-tools && python3 scripts/publish_pipeline.py ...` | wemedia subagent | wemedia subagent |
...
7. **发布主体**：wemedia subagent 直接调用 `publish_pipeline.py`（main 仅监控）
...
- 发布指令（wemedia subagent 直接执行，main 仅监控）

**main 的监控职责**：
```
wemedia 交付 → wemedia subagent 执行 → main 监控群通知兜底
```
